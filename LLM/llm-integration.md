...
Order matters because LLMs read prompts sequentially and give more weight to context that's set up early.\
<u>Generally:</u>
...
Now, keep in mind, for a multimodal message, the `content` stops being a plain string and becomes a list of parts, but it's still just the `content` field of one message (usually the `user` message) inside the same `messages` array.
...
messages=[
    {"role": "system", "content": "..."},
    {"role": "user", "content": [
        {"type": "text", "text": "The user is working on: {goal}\nTime remaining: {time_remaining} minutes\n..."},
        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{last_focus_b64}"}},
        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{distracted_b64}"}},
        {"type": "text", "text": "Write a message that does A, B, C in style Z"}
    ]}
]
